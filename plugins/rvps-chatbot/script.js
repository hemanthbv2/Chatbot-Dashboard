/**
 * RVPS Chatbot — Conversation Engine & State Machine
...
(function () {
  ...
  const STORAGE_KEY = 'rvps_chatbot_state';

  // Helper to dynamically resolve the logo path in WordPress or standalone environments
  const getLogoUrl = () => {
    return (window.rvpsChatbotSettings && window.rvpsChatbotSettings.logoUrl)
      ? window.rvpsChatbotSettings.logoUrl
      : 'https://rvps.edu.in/wp-content/uploads/2026/07/logo.png';
  };
  ...
  function loadState() {
    ...
    try {
      ...
      console.error('[RVPS Chatbot] Error loading state:', e);
      return false;
    }
  }
  ...
  function addBotMessage(html) {
    ...
    messageEl.innerHTML = `
      <div class="message-avatar"><img src="${getLogoUrl()}" alt="RV" onerror="this.onerror=null;this.src='https://rvps.edu.in/wp-content/uploads/2026/07/logo.png';"></div>
      <div>
  ...
  function addUserMessage(text) {
    if (window.rvpsTrackEvent) {
      window.rvpsTrackEvent('message', { intent: currentFlow || 'chat', query: text });
    }
    const messageEl = document.createElement('div');
  ...
  function sendToGoogleSheets(formType, formData) {
    if (window.rvpsTrackEvent) {
      window.rvpsTrackEvent('form_submit', { formType, ...formData });
    }
    // Skip if Google Sheets URL is not configured
    if (typeof GOOGLE_SHEETS_URL === 'undefined' || !GOOGLE_SHEETS_URL || GOOGLE_SHEETS_URL === '') {
      console.log('[RVPS Chatbot] Google Sheets URL not configured. Form data:', formType, formData);
      return;
    }
    ...
    // 1. Send to Web3Forms (Direct Email API)
    const web3formsPayload = {
      access_key: 'dd47b9cf-86c0-46cc-9250-699c07bc1233',
      subject: `🚨 RVPS Lead: ${payload.formType || 'General Inquiry'}`,
      from_name: 'RVPS Chatbot',
      botcheck: '', // Honeypot
      ...payload
    };
    ...
    .then(async (response) => {
      ...
      if (response.status === 200) {
        console.log('[RVPS Chatbot] Lead sent successfully to your email via Web3Forms!', json);
      } else {
        console.error('[RVPS Chatbot] Failed to send email via Web3Forms:', json);
      }
    })
    .catch(error => {
      console.error('[RVPS Chatbot] Request failed when sending lead:', error);
    });
  }
  ...
  function handleUserInput() {
    ...
    if (matchedAction) {
      if (window.rvpsTrackEvent) window.rvpsTrackEvent('message', { query: text, intent: matchedAction });
      setTimeout(() => navigateTo(matchedAction), 400);
    } else {
      // Fallback
      if (window.rvpsTrackEvent) window.rvpsTrackEvent('message', { query: text, intent: 'unmatched' });
      showTypingIndicator();
      setTimeout(() => {
        hideTypingIndicator();
        addBotMessage(
