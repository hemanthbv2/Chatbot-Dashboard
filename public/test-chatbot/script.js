...
(function () {
  ...
  const getLogoUrl = () => {
    return (window.rvcnChatbotSettings && window.rvcnChatbotSettings.logoUrl)
      ...
      : 'https://rvcn.edu.in/wp-content/uploads/2026/07/logo.png';
  };
  ...
  function addBotMessage(html) {
    ...
    messageEl.innerHTML = `
      <div class="message-avatar"><img src="${getLogoUrl()}" alt="RV" onerror="this.onerror=null;this.src='https://rvcn.edu.in/wp-content/uploads/2026/07/logo.png';"></div>
      <div>
  ...
  function addUserMessage(text) {
    if (window.rvcnTrackEvent) {
      window.rvcnTrackEvent('message', { intent: currentFlow || 'chat', query: text });
    }
    const messageEl = document.createElement('div');
  ...
  function sendToGoogleSheets(formType, formData) {
    if (window.rvcnTrackEvent) {
      window.rvcnTrackEvent('form_submit', { formType, ...formData });
    }
    ...
    const web3formsPayload = {
      ...
      subject: `🚨 RVCN Lead: ${payload.formType || 'General Inquiry'}`,
      from_name: 'RVCN Chatbot',
      botcheck: '', // Honeypot
      ...payload
    };
    ...
    .then(async (response) => {
      ...
      if (response.status === 200) {
        console.log('[RVCN Chatbot] Lead sent successfully to your email via Web3Forms!', json);
      } else {
        console.error('[RVCN Chatbot] Failed to send email via Web3Forms:', json);
      }
    })
    .catch(error => {
      console.error('[RVCN Chatbot] Request failed when sending lead:', error);
    });
  }
  ...
  function handleUserInput() {
    ...
    if (matchedAction) {
      if (window.rvcnTrackEvent) window.rvcnTrackEvent('message', { query: text, intent: matchedAction });
      setTimeout(() => navigateTo(matchedAction), 400);
    } else {
      // Fallback
      if (window.rvcnTrackEvent) window.rvcnTrackEvent('message', { query: text, intent: 'unmatched' });
      showTypingIndicator();
      setTimeout(() => {
        hideTypingIndicator();
        addBotMessage(
